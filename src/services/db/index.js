...
export function dbFactory(path: string, schemaVersionAt : number = -1): Realm {
  ...
  const latestVersion = schemaVersionAt >= 0 ? schemaVersionAt : schemata.LatestSchemaVersion;
  ...
  const currentOpts = getRealmOptions(latestVersion);
